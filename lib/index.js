...
internals.Connection.prototype.start = function (callback) {
	...
	_self.client = AzureStorage.createTableService(_self.settings.connection);
...
internals.Connection.prototype.set = function (key, value, ttl, callback) {
	...
	_self.generateRow(key, value, ttl, _self._gcfunc, function (err, insertData) {
        if (err) {
            return callback(err);
        }
    
		_self.client.insertOrMergeEntity(_self.tableName, insertData, null, function (err) {
			callback(err);
		});
	});
};
