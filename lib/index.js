...
internals.Connection.prototype.start = function(callback) {
  ...
  return AzureStorage.Validate.tableNameIsValid(self.settings.partition, (terr) => {
    if (terr) return callback(terr);

    self.tableName = self.settings.partition;

    return self.client.createTableIfNotExists(self.tableName, (err) => {
      self.azuretablegc();
      return callback(err);
    });
  });
...
internals.Connection.prototype.get = function(key, callback) {
  ...
  return self.client.retrieveEntity(self.settings.partition, key.segment, key.id, (err, record) => {
...
internals.Connection.prototype.set = function(key, value, ttl, callback) {
  ...
  return self.generateRow(key, value, ttl, self._gcfunc, (err, insertData) => {
    if (err) return callback(err);

    return self.client.insertOrMergeEntity(self.tableName, insertData, null, callback);
  });
};
...
internals.Connection.prototype.drop = function(key, callback) {
  ...
  const entGen = AzureStorage.TableUtilities.entityGenerator;
  return self.client.deleteEntity(self.tableName, {
    PartitionKey: entGen.String(key.segment),
    RowKey: entGen.String(key.id)
  }, (err) => {
    if (err) {
