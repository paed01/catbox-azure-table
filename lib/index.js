...
module.exports = function Connection(options) {
  Hoek.assert(typeof options === 'object', 'Must provide configuration to the Azure table storage cache client.');
  Hoek.applyToDefaults(defaults, options);

  const {connection, partition, ttl_interval} = options;

  Hoek.assert(typeof partition === 'string', 'Must specify a partition (Azure storage table name) to use.');
  Hoek.assert(typeof ttl_interval === 'number' || options.ttl_interval === false, 'Must provide a ttl_interval or explicitly disable it');

  const client = TableClient(connection, partition);
  let gc;
  if (ttl_interval) gc = Gc(client, options);
