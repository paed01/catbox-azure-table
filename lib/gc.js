...
internals.Gc.prototype._delete = function (entries, callback) {
	...
	_self._createBatches(entries, function (err, batches) {
		...
		async.each(Object.keys(batches), function (b, cb) {
			...
			_self.client.executeBatch(_self.tableName, batch, function (err) {
				cb();
			});
		}, callback);
	});
...
internals.Gc.prototype.collect = function (callback) {
	...
	_self._get(function (err, result, response) {
...
internals.Gc.prototype.start = function () {
	this._interval = setInterval(this.collect.bind(this), this.settings.ttl_interval);
