...
internals.Gc.prototype._delete = function (entries, callback) {
    ...
    self._createBatches(entries, function (err, batches) {
        if (err) {
            return callback(err);
        }
        ...
        async.each(Object.keys(batches), function (b, cb) {
            var batch = batches[b];
            self.client.executeBatch(self.tableName, batch, function (err) {
                cb(err);
            });
        }, function (err) {
            if (err) {
...
internals.Gc.prototype.collect = function (callback) {
    ...
    self._get(function (err, result) {
        if (err || result.entries.length === 0) {
            return innerCallback(err);
        }

        self._delete(result.entries, innerCallback);
    });
};
